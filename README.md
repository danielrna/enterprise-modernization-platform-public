...
Release: https://github.com/danielrna/enterprise-modernization-platform/releases/tag/v0.2.7

Sample smoke-test report: https://github.com/danielrna/enterprise-modernization-platform/releases/download/v0.2.7/emp-smoke-report.zip
...
```bash
docker run --rm -v "$PWD:/workspace" danielrna/enterprise-modernization-platform:v0.2.7 analyze . --pack spring-boot-3-readiness --out reports/docker-readiness
```
  ...
  readiness:
    ...
    steps:
      ...
      - name: Run EMP readiness
        uses: danielrna/enterprise-modernization-platform@v0.2.7
        with:
          path: .
...
- Validation status in benchmark reports and the Migration Hub, including 34 checkout-backed reports and 15 reports with passing compile/test evidence.
