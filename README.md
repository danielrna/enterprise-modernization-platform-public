...
Release: https://github.com/danielrna/enterprise-modernization-platform/releases/tag/v0.4.0

Sample smoke-test report: https://github.com/danielrna/enterprise-modernization-platform/releases/download/v0.4.0/emp-smoke-report.zip
...
```bash
docker run --rm -v "$PWD:/workspace" danielrna/enterprise-modernization-platform:v0.4.0 analyze . --pack spring-boot-3-readiness --out reports/docker-readiness
```
  ...
  readiness:
    ...
    steps:
      ...
      - name: Run EMP readiness
        uses: danielrna/enterprise-modernization-platform@v0.4.0
        with:
          path: .
...
- Validation status in benchmark reports and the Migration Hub, including 50 checkout-backed reports and 17 reports with passing compile/test evidence.
