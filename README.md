...
Release: https://github.com/danielrna/enterprise-modernization-platform/releases/tag/v0.1.5

Sample smoke-test report: https://github.com/danielrna/enterprise-modernization-platform/releases/download/v0.1.5/emp-smoke-report.zip
...
```bash
docker run --rm -v "$PWD:/workspace" danielrna/enterprise-modernization-platform:v0.1.5 analyze . --pack spring-boot-3-readiness --out reports/docker-readiness
```
  ...
  readiness:
    ...
    steps:
      ...
      - name: Run EMP readiness
        uses: danielrna/enterprise-modernization-platform@v0.1.5
        with:
          path: .
...
The public validation set now proves the reference flow on 20 real checkouts, including real Spring Boot applications and Hibernate ORM evidence outside Spring Guides. The validated set includes Spring Boot `2.6.2`, `2.6.3`, and `2.7.6` projects plus Hibernate reports with passing, failing, and timeout validation evidence.
...
- Validation status in benchmark reports and the Migration Hub, including 20 checkout-backed reports and 8 reports with passing compile/test evidence.
