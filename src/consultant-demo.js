...
function renderConsultantDemo(demos) {
  ...
  return page('Consultant Demo', `
    ...
    <pre><code>docker run --rm -v "$PWD:/workspace" danielrna/enterprise-modernization-platform:v0.2.3 analyze . --pack hibernate-readiness --out reports/hibernate-readiness</code></pre>
    <pre><code>docker run --rm -v "$PWD:/workspace" danielrna/enterprise-modernization-platform:v0.2.3 transform . --pack spring-boot-3-readiness --mode dry-run --validate --out reports/spring-boot-trust</code></pre>
...
\`\`\`bash
docker run --rm -v "$PWD:/workspace" danielrna/enterprise-modernization-platform:v0.2.3 analyze . --pack hibernate-readiness --out reports/hibernate-readiness
\`\`\`
`;
}
